...
@Slf4j
@Component
public class QuotationThirtyMinutesSchedule {
...
//    @Scheduled(cron = "00 */30 * * * ?")
    @SchedulerLock(name = "30mK线", lockAtMostFor = Constant.MAX_LOCK_TIME, lockAtLeastFor = Constant.MIN_LOCK_TIME)
    public void visitCountTaskByThirtyMinutes(){
        //获取间隔的开始时间和结束时间
...
//        System.out.println("30m====现在开始时间是=" + start + "结束时间是="+end);
    ...
    public JSONArray getData(String desc,long start,long end){
        ...
        String url= "";
        try{
            url = klineUrl+desc+"&interval=30m"+"&start="+start+"&end="+end+"&api_key="+apiKey;
            Object result = restTemplate.getForObject(url,Object.class);
            data = JSONArray.parseArray(JSONObject.toJSONString(result));
        } catch (Exception e) {
//            log.error("30M请求执行失败URL="+url);
            return data;
        }
        return data;
    }
