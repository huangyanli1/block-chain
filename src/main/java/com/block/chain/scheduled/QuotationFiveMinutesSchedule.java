...
@Slf4j
@Component
public class QuotationFiveMinutesSchedule {
...
//    @Scheduled(cron = "00 */5 * * * ?")
    @SchedulerLock(name = "5mK线", lockAtMostFor = Constant.MAX_LOCK_TIME, lockAtLeastFor = Constant.MIN_LOCK_TIME)
    public void visitCountTaskByFiveMinutes(){
...
//        System.out.println("5m====现在开始时间是=" + start + "结束时间是="+end);
    ...
    public JSONArray getData(String desc,long start,long end){
        ...
        String url ="";
        try{
            url = klineUrl+desc+"&interval=5m"+"&start="+start+"&end="+end+"&api_key="+apiKey;
            Object result = restTemplate.getForObject(url,Object.class);
            data = JSONArray.parseArray(JSONObject.toJSONString(result));
        } catch (Exception e) {
//            log.error("5m请求执行失败URL="+url);
            return data;
        }
        return data;
    }
